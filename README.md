...
2. then robot moves in the direction it is faacing but one step at a time unit it reached to fall off grid here what i assume is that when robot reaches to any one end of the grid which is either any sides of grip (x,4) or (4,y), it will fall it should not move after that, similar condition in the placed 
3. if the direction is north then y +=1 , if south, y-=1 , if same for east and west x +/-=1 for the movement 
4. once it reaches the end it should not be able to fall